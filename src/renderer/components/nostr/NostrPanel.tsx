...
interface Profile { pubkey:string; npub:string; name?:string; nip05?:string }
...
export default function NostrPanel({ onClose }: { onClose:()=>void }) {
  ...
  useEffect(() => {
    window.zap?.nostrGetProfile().then(setProfile)
    window.zap?.nostrGetRelays().then((r: any) => setRelays(Object.keys(r || {})))
  }, [])
    ...
    <>
      ...
      <div className="panel-body">
        {!profile ? (
          ...
          <button style={{ width:'100%', marginTop:14, padding:'9px', border:'1px solid var(--b1)', background:'none', color:'var(--t1)', borderRadius:'var(--r-sm)', cursor:'pointer', fontFamily:'var(--ff)', fontSize:12 }}
